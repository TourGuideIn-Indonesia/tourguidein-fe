...
const guideData = {
  ...
  7: { name: "Dzaky Aziz", timezone: "Europe/London" }
};
...
export default function CheckAvailability() {
  ...
  const fetchAvailableStartDate = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`/api/local-guides/${id}/available-start-date`);
  ...
  const fetchBookingOptions = async (timesToSend = {}) => {
    ...
    try {
      ...
      const response = await axios.get(`/api/local-guides/${id}/available-times`, {
        params: {
